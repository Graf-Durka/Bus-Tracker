                        ...
                        if start_idx != -1 and end_idx != -1 and start_time_str:
                            ...
                            if start_is_rel:
                                ...
                                try:
                                    ...
                                    # Корректировка даты (если сейчас вечер, а время утреннее)
                            ...
                            # Если между анкером и концом не было сброса времени (нет других автобусов)
                            # мы можем использовать анкерный метод напрямую.
                            # Проверим, убывало ли время после анкера:
                            ...
                            if not has_bus_in_between and best_anc_dt and stops[end_idx]['timeRel']:
                                match_end = re.search(r'(\d+)', stops[end_idx]['timeRel'])
                                if match_end:
                                    mins_to_end = int(match_end.group(1))
                                    end_dt = best_anc_dt + datetime.timedelta(minutes=mins_to_end)
                                    travel_mins = int((end_dt - start_dt).total_seconds() / 60)
                                    
                                    if self.is_time_valid(travel_mins, est_mins):
                                        arrival_time_end = end_dt.strftime("%H:%M")
                                        travel_time_route = travel_mins
                                        anchor_applied = True
                                        print(f"[{bus_name}] Отработал Метод 2 (Анкер). Анкер: остановка '{stops[last_anchor_idx]['name']}' в {best_anc_dt.strftime('%H:%M')}. Последняя: {mins_to_end} мин. Прибытие: {arrival_time_end}.")
                            ...
                            if not anchor_applied:
                                ...
                                for i in range(start_idx, end_idx):
                                    # Получаем минуты для текущей остановки
                                    cur_mins = None
                                    if stops[i]['timeRel']:
                                        match = re.search(r'(\d+)', stops[i]['timeRel'])
                                ...
                                if end_mins is not None:
                                    ...
                                    # Рассчитываем время прибытия на конечную относительно текущего времени (now)
                                    end_dt = now + datetime.timedelta(minutes=total_calculated_mins)
                                    travel_mins = int((end_dt - start_dt).total_seconds() / 60)
                                    
                                    if self.is_time_valid(travel_mins, est_mins):
                                        arrival_time_end = end_dt.strftime("%H:%M")
                                        travel_time_route = max(0, travel_mins)
                                        anchor_applied = True
                                        print(f"[{bus_name}] Отработал Метод 3 (Накопление). Слагаемые: {', '.join(debug_acc_stops)}. Сумма от текущего времени: {total_calculated_mins} мин. Прибытие: {arrival_time_end}.")

                            # Фолбэк, если ни одна логика не сработала (включая анкерную)
                            if not anchor_applied:
                                end_dt = start_dt + datetime.timedelta(minutes=est_mins)
                                arrival_time_end = end_dt.strftime("%H:%M")
                                travel_time_route = est_mins
                                print(f"[{bus_name}] Отработал Метод Фолбэк (На основе базы). Расчетное: {est_mins} мин. Прибытие: {arrival_time_end}.")
